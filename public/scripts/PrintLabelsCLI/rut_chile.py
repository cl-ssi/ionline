...
# Embedded file name: rut_chile.py
...
def __is_well_formatted(rut: str) -> bool:
    format_regex = "^((\\d{1,3}(\\.\\d{3})+-)|\\d+-?)(\\d|k|K)$"
...
def __generate_digit_groups(rut: str) -> []:
    digit_groups = __add_most_significant_digits_group(rut) + __generate_least_significant_digit_groups(rut)
    return digit_groups
