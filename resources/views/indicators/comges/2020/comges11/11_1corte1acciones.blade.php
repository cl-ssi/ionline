...
@for ($i = 1; $i <= 3; $i++)
        <tr class="text-center">
            <td class="text-justify">
                <ol start="{{$i}}">
                    <li>{!! $data11_1[$i]['accion'] !!}</li>
                </ol>
            </td>
            <td class="text-justify">
                <ol type="i" start="{!! $data11_1[$i]['iverificacion'] !!}">
                    <li>{!! $data11_1[$i]['verificacion'] !!}</li>
                </ol>
            </td>
            @if ($i === 1)
            <td class="align-middle text-center" rowspan="0">
            {{ $data11_1['ponderacion']  }}<br>
            {{$data11_1[$i]['anual'] }}% de la evaluación anual
            </td>
            @endif
