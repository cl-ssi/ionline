...
@section('title', 'Editar item de Emergencias y desastres '.$programming->establishment->type.' '.$programming->establishment->name.' '.$programming->year)

@section('content')

@include('programmings/nav')

<h3 class="mb-3">Editar item de Emergencias y desastres {{ $programming->establishment->type }} {{ $programming->establishment->name }} {{ $programming->year}} </h3>
<br>

<form method="POST" class="form-horizontal" action="{{ route('emergencies.update', $emergency) }}">
    @csrf
    @method('PUT')
    <div class="form-row">
        <div class="form-group col-md-2">
            ...
            <select name="origin" id="for_origin" class="form-control" required>
                ...
                <option value="Natural" {{$emergency->origin == 'Natural' ? 'selected' : ''}}>Natural</option>
                <option value="Antrópico" {{$emergency->origin == 'Antrópico' ? 'selected' : ''}}>Antrópico (Humano)</option>
            </select>
        </div>
        <div class="form-group col-md-10">
            <label for="for_name">Peligro / Amenaza</label>
            <select name="name" id="for_name" class="form-control" required>
                <option value="">Seleccione opción</option>
                @foreach($categories[$emergency->origin] as $category)
                <option value="{{$category}}" {{$category == $emergency->name ? 'selected' : ''}}>{{$category}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div id="div_for_another_name" class="form-row" @if($emergency->name != 'OTRO') style="display: none" @endif>
        <div class="form-group col-md-12">
            <fieldset class="form-group">
                <label for="for_another_name">Nombre del peligro / amenaza</label>
                <input type="text" class="form-control" id="for_another_name" name="another_name" value="{{$emergency->another_name}}">
            </fieldset>
        </div>
    </div>
    <div class="form-row">
        <div class="form-group col-md-3">
            <fieldset class="form-group">
                ...
                <a tabindex="0" id="popover-frecuency-natural"  role="button" data-toggle="popover" data-trigger="focus" data-html="true" @if($emergency->origin != 'Natural') style="display: none" @endif
                ...
                <a tabindex="0"  id="popover-frecuency-antropico" role="button" data-toggle="popover" data-trigger="focus" data-html="true" @if($emergency->origin != 'Antrópico') style="display: none" @endif
                title="Frecuencia de ocurrencia (a)" 
                ...
                <i class="fas fa-info-circle"></i></a>
                <input type="number" min="1" max="3" class="form-control" id="for_frecuency" name="frecuency" value="{{$emergency->frecuency}}" required>
            </fieldset>
        </div>
        <div class="form-group col-md-3">
            <fieldset class="form-group">
                <label for="for_impact_level">Nivel de impacto</label>
                <a tabindex="0"  id="popover-impact-level-natural" role="button" data-toggle="popover" data-trigger="focus" data-html="true" @if($emergency->origin != 'Natural') style="display: none" @endif
                ...
                <i class="fas fa-info-circle"></i></a>
                <a tabindex="0"  id="popover-impact-level-antropico" role="button" data-toggle="popover" data-trigger="focus" data-html="true" @if($emergency->origin != 'Antrópico') style="display: none" @endif
                title="Nivel de Impacto (b)" 
                data-content="Para el caso de las amenazas de Origen Antrópico, se deben considerar los siguientes criterios:<br>
                ...
                <i class="fas fa-info-circle"></i></a>
                <input type="number" min="1" max="3" class="form-control" id="for_impact_level" name="impact_level" value="{{$emergency->impact_level}}" required>
            </fieldset>
        </div>
        <div class="form-group col-md-3">
            <fieldset class="form-group">
                ...
                <i class="fas fa-info-circle"></i></a>
                <input type="number" min="1" max="3" class="form-control" id="for_ss_rol" name="ss_rol" value="{{$emergency->ss_rol}}" required>
            </fieldset>
        </div>
        <div class="form-group col-md-3">
            <fieldset class="form-group">
                ...
                <i class="fas fa-info-circle"></i></a>
                <input type="number" class="form-control" id="for_factor" name="value" value="{{$emergency->frecuency * $emergency->impact_level * $emergency->ss_rol}}" disabled>
            </fieldset>
        </div>
    </div>
    ...
    <div class="form-row">
        <div class="form-group col-md-12">
            ...
            <textarea class="form-control" id="for_description" name="description" rows="5" required>{!! $emergency->description !!}</textarea>
        </div>
    </div>
    ...
    <div class="form-row">
        <div class="form-group col-md-12">
            ...
            <textarea class="form-control" id="for_measures" name="measures" rows="5" required>{!! $emergency->measures !!}</textarea>
        </div>
    </div>
