...
@section('title', 'Bandeja de Solicitante')
      ...
      @foreach($myRequestForms as $myRequestForm)
        @foreach($myRequestForm->requestformevents as $event)
          @if(($event->type == 'status' && $event->StatusName == 'Aprobado por jefatura') ||
              ($event->type == 'message' && ($event->StatusName == 'Aprobado por jefatura - Registro Item' || $event->StatusName == 'Aprobado por jefatura - Registro CRP')))
            @if($loop->last)
            <tr>
