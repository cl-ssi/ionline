<div>
    @if($mammography->first_dose_at)
        <h5>Primera dósis:</h5>
        <p>Suministrada el día <strong>{{ $mammography->first_dose_at->format('d-m-Y') }}.</strong></p>
    ...
    @if($mammography->first_dose_at)
        @if($mammography->second_dose_at)
            <h5>Segunda dósis:</h5>
            <p>Suministrada el día <strong>{{ $mammography->second_dose_at->format('d-m-Y \a \l\a\s H:i') }} horas.</strong></p>

        @elseif($mammography->second_dose)
            <h5>Segunda dósis:</h5>
            <p>Programada para el día <strong>{{ $mammography->second_dose->format('d-m-Y \a \l\a\s H:i') }} horas.</strong>
            Debe llegar 10 minutos ántes.</p>
