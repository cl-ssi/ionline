...
@section('content')
    ...
    <h3 class="mb-3">Ver detalle de Convenio #{{ $agreement->id }}
        @can('Agreement: delete')
            <form method="POST" action="{{ route('agreements.destroy', $agreement->id) }}"
                onclick="return confirm('¿Está seguro/a de eliminar este convenio?');" class="d-inline">
    ...
    <ol class="breadcrumb bg-light justify-content-end small">
        @if ($agreement->period >= 2024)
            @if ($canEdit)
                @if ($agreement->program_id == 3)
                    <li class="nav-item">
                        ...
                        <form id="submit-form" action="{{ route('agreements.createDocument', $agreement) }}" method="POST"
                            class="hidden">
                            @csrf
                            @method('POST')
                        </form>
                ...
                @if ($agreement->document_id != null)
                    <li class="nav-item">
                        <a href="{{ route('documents.edit', $agreement->document_id) }}" class="nav-link text-secondary"><i
                                class="fas fa-file-alt"></i> Editar borrador Convenio</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ route('documents.show', $agreement->document_id) }}" class="nav-link text-secondary"><i
                                class="fas fa-eye"></i> Ver borrador Convenio</a>
                    </li>
                @endif
            @endif
                ...
                @if ($canEdit && $agreement->document_id != null)
                    <li class="nav-item">
                        <a class="nav-link text-secondary"
                            href="{{ route('documents.sendForSignature', $agreement->document_id) }}"><i
                                class="fas fa-file-signature"></i> Solicitar visación Convenio</a>
                    </li>
                @endif
            @endcan
            ...
            @if ($agreement->document?->fileToSign)
                <li class="nav-item">
                    @if ($agreement->document->fileToSign->HasAllFlowsSigned)
                        <a href="{{ route('documents.signedDocumentPdf', $agreement->document_id) }}"
            ...
            @canany(['Documents: signatures and distribution'])
                @if ($canEdit && $agreement->document?->fileToSign?->HasAllFlowsSigned)
                    <li class="nav-item">
                        <a class="nav-link text-secondary" href="{{ route('agreements.sign', [$agreement, 'signer']) }}"><i
                                class="fas fa-file-signature"></i> Solicitar firma Convenio</a>
                    </li>
                @endif
            @endcan
            ...
            @if ($agreement->fileToSign?->HasAllFlowsSigned)
                <li class="nav-item">
                    <a class="nav-link text-secondary"
                        href="{{ route('documents.signatures.showPdf', [$agreement->file_to_sign_id, time()]) }}"
                        target="blank"><i class="fas fa-eye"></i> Ver convenio firmado</a>
                </li>
                @if ($canEdit)
                    @if ($agreement->fileToSign?->HasAllFlowsSigned)
                        {{-- <li class="nav-item">
                    <a href="#" class="nav-link text-secondary" data-toggle="modal"
                        ...
                        @if ($agreement->res_document_id != null)
                            <li class="nav-item">
                                <a href="{{ route('documents.edit', $agreement->res_document_id) }}"
                                    class="nav-link text-secondary"><i class="fas fa-file-alt"></i> Editar borrador
                                    Resolución Convenio</a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('documents.show', $agreement->res_document_id) }}"
                                    class="nav-link text-secondary"><i class="fas fa-eye"></i> Ver borrador Resolución
                                    Convenio</a>
                            </li>
                        @endif
                    @endif
        ...
        @else
            ...
            @if ($canEdit)
                <li class="nav-item">
                    @if ($agreement->program_id == 3)
                        <!-- convenio retiro voluntario -->
                        <a class="nav-link text-secondary"
                            href="{{ route('agreements.createWordWithdrawal', $agreement) }}"><i
                                class="fas fa-file-download"></i> Descargar borrador Convenio</a>
                    @elseif($agreement->program_id == 50)
                        <!-- convenio de colaboración -->
                        <a class="nav-link text-secondary"
                            href="{{ route('agreements.createWordCollaboration', $agreement) }}"><i
                                class="fas fa-file-download"></i> Descargar borrador Convenio</a>
                    @elseif($agreement->isTypeMandate() && $agreement->period != 2023)
                        <!-- convenio mandato para program id 44 con componente de desarrollo rrhh, para periodo 2023 corre convenio general -->
                        <a class="nav-link text-secondary"
                            href="{{ route('agreements.createWordMandate', $agreement) }}"><i
                                class="fas fa-file-download"></i> Descargar borrador Convenio</a>
                    @elseif($agreement->program_id == 51)
                        <!-- convenio mandato PFC para program id 51 -->
                        <a class="nav-link text-secondary"
                            href="{{ route('agreements.createWordMandatePFC', $agreement) }}"><i
                                class="fas fa-file-download"></i> Descargar borrador Convenio</a>
                    ...
                    @else
                        <a class="nav-link text-secondary" href="{{ route('agreements.createWord', $agreement) }}"><i
                                class="fas fa-file-download"></i> Descargar borrador Convenio</a>
                    @endif
                </li>
                ...
                @if ($agreement->file != null)
                    <li>
                        <a class="nav-link text-secondary" href="{{ route('agreements.preview', $agreement->id) }}"
                            target="blank"><i class="fas fa-eye"></i> Previsualizar convenio</a>
                    </li>
                @endif
            ...
            @canany(['Documents: signatures and distribution'])
                @if ($canEdit)
                    <li class="nav-item">
                        <a class="nav-link text-secondary" href="{{ route('agreements.sign', [$agreement, 'visators']) }}"><i
                                class="fas fa-file-signature"></i> Solicitar visación Convenio</a>
                    </li>
                @endif
                ...
                @if ($agreement->fileToEndorse && $agreement->fileToEndorse->HasAllFlowsSigned)
                    <li class="nav-item">
                        <a class="nav-link text-secondary"
                            href="{{ route('documents.signatures.showPdf', [$agreement->file_to_endorse_id, time()]) }}"
                            target="blank"><i class="fas fa-eye"></i> Ver convenio visado</a>
                    </li>
                    @if ($canEdit)
                        <li class="nav-item">
                            <a class="nav-link text-secondary"
                                href="{{ route('agreements.sign', [$agreement, 'signer']) }}"><i
                                    class="fas fa-file-signature"></i> Solicitar firma Convenio</a>
                        </li>
                    @endif
                @endif
                        ...
                        @else
                            <li class="nav-item">
                                <div class="tooltip-wrapper disabled"
                                    data-title="No existe registro de archivo docx versión final de Covenio Referentes, adjuntelo para habilitar esta opción">
                                    <a href="#" class="nav-link text-secondary disabled"><i
                                            class="fas fa-file-download"></i> Descargar borrador Resolución</a>
                                </div>
                            </li>
                        @endif
                    @endif
                ...
                @if ($agreement->fileResEnd != null)
                    <li class="nav-item">
                        <a class="nav-link text-secondary" href="{{ route('agreements.downloadRes', $agreement->id) }}"
                            target="blank"><i class="fas fa-eye"></i> Ver resolución firmada</a>
                    </li>
                @endif
            @endcan
        @endif
    ...
    <div id="accordion" class="small">
        <div class="card">
            <div class="card-header" id="headingOne">
                <h5 class="mb-0">
                    <button class="btn btn-xs" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true"
            ...
            <div id="collapseOne" class="collapse {{ in_array($agreement->program_id, [3, 50]) ? 'show' : '' }}"
                aria-labelledby="headingOne" data-parent="#accordion">
                <div class="card-body">
                    <form method="POST" class="form-horizontal" enctype="multipart/form-data"
                        action="{{ route('agreements.update', $agreement->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-row">
                            ...
                            <fieldset class="form-group col-3">
                                <label for="for">Archivo DOCX
                                    @if ($agreement->file != null)
                                        <a class="text-info" href="{{ route('agreements.download', $agreement->id) }}"
                                            target="_blank">
                                ...
                                <input type="file" class="form-control-file" id="forfile" name="file"
                                    accept=".doc,.docx" @if (!$canEdit) disabled @endif>
                                <small class="form-text text-muted">* Adjuntar versión final de Convenio formato
                                    docx</small>
                        ...
                        <div class="form-row">
                            <fieldset class="form-group col-2">
                                ...
                                <input type="date" name="date" class="form-control" id="fordate"
                                    value="{{ $agreement->date?->format('Y-m-d') }}"
                            ...
                            @if ($agreement->program_id == 3)
                                <fieldset class="form-group col-2">
                                    ...
                                    <input type="integer" class="form-control" id="forquotas" name="quotas"
                                        min="1" value="{{ $agreement->quotas }}" required
                                        @if (!$canEdit) disabled @endif>
                                </fieldset>
                                <fieldset class="form-group col-3">
                                    <label for="fortotal_amount">Monto total</label>
                                    <input type="integer" class="form-control" id="fortotal_amount" name="total_amount"
                                        min="100" value="{{ $agreement->total_amount }}" required
                                        @if (!$canEdit) disabled @endif>
                                </fieldset>
                            @else
                                <fieldset class="form-group col-4">
                                    <label for="forestablishment">Centros de Atencion</label>
                                    <select id="establishment" class="selectpicker " name="establishment[]"
                                        title="Seleccionar" data-selected-text-format="count > 2" data-width="100%"
                                        multiple @if (!$canEdit)
                                        disabled
                            @endif>
                            @foreach ($agreement->commune->establishments as $key => $establishment)
                                <option value="{{ $establishment->id }}">{{ $establishment->type }} -
                        ...
                        <div class="form-row">
                            <fieldset class="form-group col-12">
                                ...
                                <select name="director_signer_id" class="form-control selectpicker" title="Seleccione..."
                                    required @if (!$canEdit) disabled @endif>
                                    @foreach ($signers as $signer)
                                        <option value="{{ $signer->id }}"
                                            @if ($signer->id == $agreement->director_signer_id) selected @endif>{{ $signer->appellative }}
                                            {{ $signer->user->fullName }}, {{ $signer->decree }}</option>
                                    @endforeach
                                </select>
                            </fieldset>
                        </div>
                        <div class="form-row">
                            <fieldset class="form-group col-4">
                                ...
                                <select id="representative" class="selectpicker" name="representative"
                                    title="Seleccionar" data-width="100%"
                                    @if (!$canEdit) disabled @endif>
                                    <option value="{{ $municipality->name_representative }}"
                                        @if ($municipality->name_representative == $agreement->representative) selected @endif>
                                        {{ $municipality->name_representative }}</option>
                                    @if ($municipality->name_representative_surrogate != null)
                                        <option value="{{ $municipality->name_representative_surrogate }}"
                                            @if ($municipality->name_representative_surrogate == $agreement->representative) selected @endif>
                                            {{ $municipality->name_representative_surrogate }}</option>
                                    @endif
                                    @if (
                                        $agreement->representative != null &&
                                            $agreement->representative != $municipality->name_representative &&
                                            $agreement->representative != $municipality->name_representative_surrogate)
                                        <option value="{{ $agreement->representative }}" selected>
                                            {{ $agreement->representative }}</option>
                                    @endif
                                </select>
                            ...
                            <fieldset class="form-group col-2">
                                ...
                                <input type="text" name="representative_rut" class="form-control"
                                    id="representative_rut" value="{{ $agreement->representative_rut }}" readonly>
                            </fieldset>
                            <fieldset class="form-group col-6">
                                <label for="fornumber">Decreto representante</label>
                                <input type="text" name="representative_decree" class="form-control"
                                    id="representative_decree" value="{{ $agreement->representative_decree }}" readonly>
                            </fieldset>
                            <input type="hidden" name="representative_appelative" id="representative_appelative"
                                value="{{ $agreement->representative_appelative }}">
                        </div>

                        <div class="form-row">
                            <fieldset class="form-group col-4">
                                <label for="fornumber">Municipio</label>
                                <input type="integer" name="municipality_name" class="form-control"
                                    id="municipality_name" value="{{ $municipality->name_municipality }}" readonly>
                            </fieldset>
                            <fieldset class="form-group col-2">
                                <label for="fornumber">Rut Municipalidad</label>
                                <input type="integer" name="municipality_rut" class="form-control" id="municipality_rut"
                                    value="{{ $agreement->municipality_rut }}" readonly>
                            </fieldset>
                            <fieldset class="form-group col-6">
                                <label for="fornumber">Dirección Municipalidad</label>
                                <input type="integer" name="municipality_adress" class="form-control"
                                    id="municipality_adress" value="{{ $agreement->municipality_adress }}" readonly>
                            </fieldset>
                        </div>
                        @if ($agreement->program_id != 50)
                            ...
                            <div class="form-row">
                                <fieldset class="form-group col-3">
                                    ...
                                    <input type="integer" name="number" class="form-control" id="fornumber"
                                        value="{{ $agreement->number }}"
                                        @if (!$canEdit) disabled @endif>
                                    <small class="form-text text-muted">* Nro. Resolución, se puede agregar al
                                        final.</small>
                                </fieldset>

                                <fieldset class="form-group col-3">
                                    <label for="fordate">Fecha Resolución del Programa Ministerial</label>
                                    <input type="date" name="resolution_date" class="form-control" id="fordate"
                                        value="{{ $agreement->resolution_date }}"
                                        @if (!$canEdit) disabled @endif>
                                </fieldset>
                                ...
                                @if ($agreement->program_id != 3)
                                    <fieldset class="form-group col-3">
                                        ...
                                        <input type="integer" name="res_resource_number" class="form-control"
                                            id="fornumber" value="{{ $agreement->res_resource_number }}"
                                            @if (!$canEdit) disabled @endif>
                                        <small class="form-text text-muted">* Nro. Resolución, se puede agregar al
                                            final.</small>
                                    </fieldset>
                                    ...
                                    <fieldset class="form-group col-3">
                                        ...
                                        <input type="date" name="res_resource_date" class="form-control"
                                            id="fordate" value="{{ $agreement->res_resource_date }}"
                                            @if (!$canEdit) disabled @endif>
                                    </fieldset>
                                @endif
                            </div>
                        ...
                        <div class="form-row">
                            <fieldset class="form-group col-3">
                                ...
                                <input type="integer" name="res_exempt_number" class="form-control" id="fornumber"
                                    value="{{ $agreement->res_exempt_number }}"
                                    @if (!$canEdit) disabled @endif>
                                <small class="form-text text-muted">* Nro. Resolución Exenta, se puede agregar al
                                    final.</small>
                            </fieldset>

                            <fieldset class="form-group col-3">
                                <label for="fordate">Fecha Resolución Exenta del Convenio</label>
                                <input type="date" name="res_exempt_date" class="form-control" id="fordate"
                                    value="{{ $agreement->res_exempt_date?->format('Y-m-d') }}"
                                    @if (!$canEdit) disabled @endif>
                            </fieldset>
                            ...
                            <fieldset class="form-group col-3">
                                <label for="for">Archivo Resolución Final PDF SSI
                                    @if ($agreement->fileResEnd != null)
                                        <a class="text-info" href="{{ route('agreements.downloadRes', $agreement->id) }}"
                                            target="_blank">
                                            <i class="fas fa-paperclip"></i> adjunto
                                        </a>
                                    @endif
                                </label>
                                <input type="file" class="form-control-file" id="forfileResEnd" name="fileResEnd"
                                    accept=".pdf" @if (!$canEdit) disabled @endif>
                                <small class="form-text text-muted">* Adjuntar versión final de Resolución Función
                                    Exclusiva Encargado de Convenio</small>
    ...
    @if (!in_array($agreement->program_id, [3, 50]))
                                ...
                                @if ($canEdit)
                                    <td class="text-right">
                                        <button class="btn btn-sm btn-outline-secondary" data-toggle="modal"
                                            ...
                                            data-subtitle="{{ $amount->subtitle }}" data-amount="{{ $amount->amount }}"
                                            data-formaction="{{ route('agreements.amount.update', $amount->id) }}">
                                            <i class="fas fa-edit"></i></button>

                                        <form method="POST"
                                            action="{{ route('agreements.amount.destroy', $amount->id) }}"
                                            class="d-inline">
                                            {{ method_field('DELETE') }} {{ csrf_field() }}
                                            <button class="btn btn-sm btn-outline-danger"
                                                onclick="return confirm('¿Desea eliminar el componente realmente?')"><i
                                                    class="fas fa-trash-alt"></i></button></button>
                                        </form>
                                    </td>
                                @endif
                            </tr>
        ...
        <div class="card mt-3 small">
            <div class="card-body">
                <h5>Cuotas
                    @if ($canEdit)
                        <form method="POST" action="{{ route('agreements.quotaAutomatic.update', $agreement->id) }}"
                            class="d-inline float-right small">
                            {{ method_field('PUT') }} {{ csrf_field() }}
                            <button class="btn btn-sm btn-outline-primary"
                                onclick="return confirm('¿Desea realmente calcular las cuotas automaticamente?')"><i
                                    class="fas fa-sync"></i> Calculo Automático</button></button> <!-- onclick="return confirm('¿Desea eliminar el componente realmente?')-->
                </form>
@endif
            </h5>
                            ...
                            @canany(['Documents: signatures and distribution'])
                                @if ($canEdit)
                                    <a class="btn btn-sm btn-outline-secondary"
                                        href="{{ route('agreements.addendum.sign', [$addendum, 'visators']) }}"
                                        data-toggle="tooltip" data-placement="top" title="Solicitar visación Addendum"><i
                                            class="fas fa-file-signature"></i></a>
                                @endif
                                @if ($addendum->fileToEndorse && $addendum->fileToEndorse->HasAllFlowsSigned)
                                    <a class="btn btn-sm btn-outline-secondary"
                                        href="{{ route('documents.signatures.showPdf', [$addendum->file_to_endorse_id, time()]) }}"
                                        target="blank" data-toggle="tooltip" data-placement="top"
                                        title="Ver addendum visado"><i class="fas fa-eye"></i></a>
                                    @if ($canEdit)
                                        <a class="btn btn-sm btn-outline-secondary"
                                            href="{{ route('agreements.addendum.sign', [$addendum, 'signer']) }}"
                                            data-toggle="tooltip" data-placement="top" title="Solicitar firma Addendum"><i
                                                class="fas fa-file-signature"></i></a>
                                    @endif
                                @endif
                                ...
                                @if ($addendum->fileToEndorse && $addendum->fileToEndorse->HasAllFlowsSigned)
                                    @if ($addendum->fileToSign && $addendum->fileToSign->HasAllFlowsSigned)
                                        <a class="btn btn-sm btn-outline-secondary"
                                            href="{{ route('documents.signatures.showPdf', [$addendum->file_to_sign_id, time()]) }}"
                                            target="blank" data-toggle="tooltip" data-placement="top"
                                    ...
                                    @if ($addendum->file)
                                        @if ($canEdit)
                                            <span data-toggle="modal" data-target="#selectSignerRes" data-formmethod="POST"
                                                data-formaction="{{ route('agreements.addendum.createWord' . ($agreement->program_id == 3 ? 'Withdrawal' : ''), [$addendum, 'res']) }}">
                                                <a href="#" class="btn btn-sm btn-outline-secondary"
                                                    data-toggle="tooltip" data-placement="top"
                                                    title="Descargar borrador Resolución Addendum"><i
                                                        class="fas fa-fw fa-file-word"></i></a>
                                            </span>
                                        @endif
                                    @endif
                            ...
                            <tr>
                                ...
                                <td>{{ $continuity->res_number }}
                                    @if ($continuity->res_file)
                                        <a class="text-info"
                                            href="{{ route('agreements.continuity.downloadRes', $continuity) }}"
                                            target="_blank">
                                            <i class="fas fa-paperclip"></i>
                                        </a>
                                    @endif
                                ...
                                <td>
                                    @if ($canEdit)
                                        <button class="btn btn-sm btn-outline-secondary" data-toggle="modal"
                                            data-target="#editModalContinuityResol-{{ $continuity->id }}">
                                            <i class="fas fa-edit"></i></button>
                                ...
                                <form id="submit-form-continuity-{{$continuity->id}}" action="{{ route('agreements.continuity.createWord', $continuity) }}" method="POST" hidden>@csrf</form> --}}
                                        <a class="btn btn-sm btn-outline-secondary" href="#" data-toggle="tooltip"
                                            data-placement="top" title="Generar nuevo borrador Resolucion"
                                            onclick="event.preventDefault(); document.getElementById('submit2-form-continuity-{{ $continuity->id }}').submit();"><i
                                                class="fas fa-file-medical"></i></a>
                                        <form id="submit2-form-continuity-{{ $continuity->id }}"
                                            action="{{ route('agreements.continuity.createDocument', $continuity) }}"
                                            method="POST" hidden>@csrf</form>
                                ...
                                @endif --}}
                                        @if ($continuity->document_id != null)
                                            <a href="{{ route('documents.edit', $continuity->document_id) }}"
                                                class="btn btn-sm btn-outline-secondary" target="_blank"
                                                data-toggle="tooltip" data-placement="top"
                                                title="Editar borrador Resolución"><i class="fas fa-file-alt"></i></a>
                                            <a href="{{ route('documents.show', $continuity->document_id) }}"
                                                class="btn btn-sm btn-outline-secondary" target="_blank"
                                                data-toggle="tooltip" data-placement="top"
                                                title="Visualizar borrador Resolución"><i class="fas fa-eye"></i></a>
                                        @endif
                                    @endif <!-- canEdit fin -->
                                    @canany(['Documents: signatures and distribution'])
                                        @if ($canEdit && $continuity->document_id != null)
                                            <a class="btn btn-sm btn-outline-secondary"
                                                href="{{ route('documents.sendForSignature', $continuity->document_id) }}"
                                                data-toggle="tooltip" data-placement="top"
                                                title="Solicitar visación Resolucion"><i
                                                    class="fas fa-file-signature"></i></a>
                                        @endif
                            ...
                            @endif --}}
                                    ...
                                    @if ($continuity->document && $continuity->document->fileToSign)
                                        @if ($continuity->document->fileToSign->hasSignedFlow)
                                            <a href="{{ route('documents.signedDocumentPdf', $continuity->document_id) }}"
                                                class="btn btn-sm @if ($continuity->document->fileToSign->hasAllFlowsSigned) btn-outline-success @else btn-outline-primary @endif"
                                                target="_blank" data-toggle="tooltip" data-placement="top"
                                                title="Ver resolución visado">
...
@section('custom_js')
    ...
    <script type="text/javascript">
        ...
        $('#representative').on('change', function(e) {
            ...
            if (@json($agreement->representative) != null && @json($agreement->representative) !=
                @json($municipality->name_representative) && @json($agreement->representative) != @json($municipality->name_representative_surrogate)) {
