...
<form method="POST" enctype="multipart/form-data" action="{{ route('rrhh.service-request.store') }}">
	...
	<div class="row">
		...
		<fieldset class="form-group col">
			...
			<input type="date" class="form-control" id="for_request_date" name="request_date" max="{{$anosiguiente}}-12-31" required>
		</fieldset>
		

		<fieldset class="form-group col">
			<label for="for_start_date">F.Inicio de Contrato</label>
			<input type="date" class="form-control" id="for_start_date" name="start_date" min="{{$ano}}-01-01" max="{{$anosiguiente}}-12-31" required>
		</fieldset>

		<fieldset class="form-group col">
			<label for="for_end_date">F.Término de Contrato</label>
			<input type="date" class="form-control" id="for_end_date" name="end_date" min="{{$ano}}-01-01" max="{{$anosiguiente}}-12-31" required>
		</fieldset>
...
<script type="text/javascript">
	...
	$('#type').on('change', function() {
		...
		if (value == "Suma alzada") {
			...
			if ({
					{
						Auth::user() - > organizationalUnit - > establishment_id
					}
				} == 1) {
				$("#programm_name option[value='PRAPS']").hide();
				$("#programm_name option[value='PESPI']").hide();
	...
	$('#btn_fonasa').click(function() {
		...
		var dv = $("#for_dv").val();
		var url = '{{route('
		webservices.fonasa ')}}/?run=' + run + '&dv=' + dv;
