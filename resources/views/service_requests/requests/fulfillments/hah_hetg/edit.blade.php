...
<!-- solo accederan para modificar la info a los rrhh de hah y finanzas de hetg -->
@if((auth()->user()->can('Service Request: fulfillments rrhh') && auth()->user()->establishment_id != 41) ||
    (auth()->user()->can('Service Request: fulfillments finance') && auth()->user()->establishment_id != 1))
    
    @if($serviceRequest->program_contract_type == "Mensual")
        @include('service_requests.requests.fulfillments.hah_hetg.edit_monthly',['serviceRequest' => $serviceRequest])
    @else

        @if($serviceRequest->programm_name == "Covid 2022")
        @if($serviceRequest->working_day_type == "HORA MÉDICA")
            @include('service_requests.requests.fulfillments.hah_hetg.edit_hours_medics',['serviceRequest' => $serviceRequest])
        @else
            @include('service_requests.requests.fulfillments.hah_hetg.edit_hours_others',['serviceRequest' => $serviceRequest])
        @endif
        @else
