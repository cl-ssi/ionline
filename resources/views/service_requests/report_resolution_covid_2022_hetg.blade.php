  ...
  <div class="content">
    ...
    <div class="content">
      ...
      <p class="justify">
        ...
        las facultades que me confiere el {{App\Models\Rrhh\Authority::getAuthorityFromDate(1,now(),['manager'])->decree}};
        @endif
      ...
      <p class="justify">
        ...
        Don {{App\Models\Rrhh\Authority::getAuthorityFromDate(84,now(),['manager'])->user->FullNameUpper}}, en su calidad de {{App\Models\Rrhh\Authority::getAuthorityFromDate(84,now(),['manager'])->position}} del Hospital “Dr. Ernesto Torres Galdames” de Iquique, contrata los servicios a honorarios a suma alzada de {{$ServiceRequest->employee->getFullNameAttribute()}}, ({{$ServiceRequest->profession->name}} - {{$ServiceRequest->working_day_type}}), para que preste servicios en el {{$ServiceRequest->responsabilityCenter->name}} al fin de cubrir la recarga de las tareas provocadas por la pandemia de COVID-19.
      </p>
      ...
      @else
        ...
        @elseif($ServiceRequest->working_day_type == "HORA EXTRA")
            <p class="justify">
                ...
                <!-- 31/10/2023: Samantha/Héctor solicitan cambio en formato para contratos 'HORA EXTRA' desde el 1 de septiembre  -->
                @if($ServiceRequest->start_date->format('Y-m-d') >= '2023-09-01')
                    @livewire('service-request.show-total-hours', ['fulfillment' => $ServiceRequest->fulfillments->first(),
                                                                    'forResolution' => true])
                @else
                    {{number_format($ServiceRequest->Fulfillments->first()->total_hours_to_pay)}} Horas 
                @endif por extensión horaria en el mes 
                de {{$ServiceRequest->start_date->monthName}}, cuya suma alzada total es de ${{number_format($ServiceRequest->Fulfillments->first()->total_to_pay)}}.- ({{$ServiceRequest->fulfillments->first()->total_to_pay_description}}) impuesto incluido, en conformidad a lo dispuesto en el inciso segundo del Art. 2º del Decreto Nº 98 de 1991 del Ministerio de Hacienda y se cancelará en una cuota de ${{number_format($ServiceRequest->Fulfillments->first()->total_to_pay)}} el mes de {{$ServiceRequest->start_date->monthName}}; se deberá acreditar contra presentación de certificado extendido por el Jefe del {{$ServiceRequest->responsabilityCenter->name}}, dependiente del
                Hospital Regional de Iquique,
