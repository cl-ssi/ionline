    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav mr-auto">
                ...
                <li
                    class="nav-item dropdown {{ active(['indicators.*', 'programmings.*', 'communefiles.index', 'le_extra_plans.index']) }}">
                ...
                @auth
                    ...
                    @can('Requirements: create')
                        <li class="nav-item {{ active('requirements.*') }}">
                            <a class="nav-link" href="{{ route('requirements.inbox') }}">
                                ...
                                <span
                                    class="badge badge-secondary">{{ App\Models\Requirements\Requirement::getPendingRequirements() }}</span>
                            </a>
                        </li>
                    @endcan
                    ...
                    @if (auth()->user()->organizationalUnit)
                        <li
                            class="nav-item dropdown {{ active(['request_forms.*', 'warehouse.*', 'pharmacies.*', 'resources.*', 'inventories.*', 'allowances.*']) }}">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                ...
                                @if (auth()->user()->organizationalUnit->establishment_id != 1)
                                    ...
                                    @if (auth()->user()->organizationalUnit &&
                                            in_array(auth()->user()->organizationalUnit->establishment_id, explode(',', $ouSearch)))
                                        <a class="dropdown-item {{ active('request_forms.my_forms') }}"
                                    ...
                                    @canany(['Agenda UST: Administrador'])
                                        <a class="dropdown-item {{ active('prof_agenda.home') }}"
                                    ...
                                    @canany(['Store', 'Store: admin', 'Store: index', 'Store: list receptions', 'Store: list
                                        dispatchs', 'Store: bincard report', 'Store: maintainer programs', 'Store: maintainers',
                                        'Store: add invoice', 'Store: create dispatch', 'Store: create reception by donation',
                                        'Store: create reception by purcharse order'])
                            ...
                            @endcan --}}
                                    ...
                                    @canany(['Store: warehouse manager'])
                                        <a class="dropdown-item {{ active('warehouse.stores.index') }}"
                                    ...
                                    @canany(['Store: Cenabast'])
                                        <a class="dropdown-item {{ active('warehouse.cenabast.index') }}"
                                            href="{{ route('warehouse.cenabast.index') }}">
                                    ...
                                    @can('Inventory')
                                        ...
                                        <a class="dropdown-item" href="{{ route('medical-equipment.index') }}">
                                            <i class="fas fa-fw fa-x-ray"></i> Administrar Equipos
                                        </a>
                                    @endcan
                                    ...
                                    @can('Inventory')
                                        @foreach (auth()->user()->establishmentInventories as $establishmentItem)
                                            <a class="dropdown-item"
                                                href="{{ route('inventories.index', $establishmentItem) }}">
                                ...
                                @if (auth()->user()->organizationalUnit->establishment_id != 1)
                                    ...
                                    {{-- @if (Auth::user()->can('Allowances: create') || Auth::user()->can('Allowances: all') || Auth::user()->can('Allowances: reports')) --}}

                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item {{ active('allowances.index') }}"
                    ...
                    @canany(['Users: create', 'Users: edit', 'Users: delete', 'OrganizationalUnits: create',
                        'OrganizationalUnits: edit', 'OrganizationalUnits: delete', 'Authorities: view', 'Authorities: create',
                        'Users: service requests', 'Service Request', 'Replacement Staff: create request', 'Replacement Staff:
                        view requests', 'Job Position Profile: create', 'Job Position Profile: all', 'Job Position Profile:
                        review'])
                        <li
                    ...
                    {{ active(['rrhh.users.*', 'rrhh.organizational-units.*', 'rrhh.authorities.*', 'suitability.*', 'replacement_staff.request.*']) }}">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="d-inline-block" tabindex="0" data-toggle="tooltip"
                                    title="Subdirección de Gestión y Desarrollo de las Personas">
                            ...
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                ...
                                @canany(['OrganizationalUnits: create', 'OrganizationalUnits: edit', 'OrganizationalUnits:
                                    delete'])
                                ...
                                @canany(['Authorities: view', 'Authorities: create'])
                                    <a class="dropdown-item {{ active('rrhh.new-authorities.*') }}"
                                        href="{{ route('rrhh.new-authorities.index') }}">
                                ...
                                @canany(['Rrhh: birthday'])
                                    <a class="dropdown-item {{ active('rrhh.users.birthdayGrettings') }}"
                                        href="{{ route('rrhh.users.birthdayGrettings') }}">
                        ...
                        @endif --}}
                                ...
                                @if (Auth::user()->manager->count() > 0 ||
                                    ...
                                    @if (Auth::user()->manager->count() > 0 ||
                                            ...
                                            Auth::user()->can('Job Position Profile: all') ||
                                            Auth::user()->can('Job Position Profile: review'))
                                        <a class="dropdown-item {{ active('job_position_profile.index') }}"
                                            href="{{ route('job_position_profile.index') }}">
                    ...
                    @can('Mammography: admin')
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('mammography.index') }}" title="Mamografias"> <i
                                    class="fas fa-fw fa-dot-circle"></i> </a>
                        </li>
                    @endcan
            ...
            <ul class="navbar-nav ml-auto">
                ...
                @else
                    @canany(['be god', 'Parameters: programs', 'Parameters: professions', 'Parameters: locations',
                        'Parameters: places', 'Parameters: holidays', 'Parameters: UNSPSC'])
                        <li class="nav-item dropdown {{ active(['parameters.*']) }}">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                @canany(['be god'])
                                    <a class="dropdown-item {{ active('rrhh.absence-types.*') }}"
                                        href="{{ route('rrhh.absence-types.index') }}">
                                ...
                                @canany(['be god'])
                                    ...
                                    <a class="dropdown-item {{ active('rrhh.users.last-access') }}"
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link" href="#" role="button" data-toggle="dropdown" aria-haspopup="true"
                            ...
                            @if (count(auth()->user()->unreadNotifications))
                                <span
                                    class="badge badge-secondary">{{ count(auth()->user()->unreadNotifications) }}</span>
                            @endif
                        </a>
                    ...
                    <li class="nav-item dropdown">
                        <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button"
                            ...
                            @if (auth()->user()->gravatar)
                                <img src="{{ auth()->user()->gravatarUrl }}?s=30&d=mp&r=g" class="rounded-circle"
