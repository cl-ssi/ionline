...
class WordMandateAgreeController extends Controller
{
    public function createWordDocx($id)
    {
    	...
    	$templateProcesor = new \PhpOffice\PhpWord\TemplateProcessor(public_path('word-template/conveniomandato'.$agreements->period.'.docx'));
        ...
        //CLONE BLOCK PARA 2 CUOTAS
        foreach ($arrayQuota as $key => $item){
            foreach ($item as $search => $replace){
                $templateProcesor->setValue($search."#".($key+1), $replace, 1);
            }
        }

    	$templateProcesor->saveAs(storage_path('app/public/Prev-Conv-Mandato.docx'));

    	return response()->download(storage_path('app/public/Prev-Conv-Mandato.docx'))->deleteFileAfterSend(true);
    }
